...
- Control-API port = `devtooie.yaml` `apiPort` (default `4099` if absent). No env var,
  no flag. The agent skill / external callers read the port straight from `devtooie.yaml`.
...
| `--logfile <path>` | Write all logs to this file. Defaults to `node_modules/.devtooie/devtooie.log` (truncated per session). |
...
- **Control-API port** — `getApiPort()`: `devtooie.yaml` `apiPort` → `4099`.
  **[portability]** No env var, no `.env.*` parsing, no CLI flag — the port lives only
  in `devtooie.yaml`.
- **Persistence** — read/write/reset the saved selection. **[portability]** Stored
  under `node_modules/.devtooie/selection.json` (never inside the `devtooie` package's
  own dir); see §10.
...
**Ephemeral, per-machine → `node_modules/.devtooie/`** (a scratch dir *beside* the
package symlinks — never the `devtooie` package's own dir, which under pnpm is a
symlink into the shared, reinstall-wiped store). Already git-ignored because
`node_modules` is; nothing to add. Nothing here is written via the package's own
`import.meta.dirname`:
- **Saved selection** → `node_modules/.devtooie/selection.json`.
- **Default logfile** → `node_modules/.devtooie/devtooie.log` (override with `--logfile`,
  e.g. to put it at the repo root). Truncated per session.
- **Debug log** (opt-in via `DEBUG_DEVTOOIE`) → `node_modules/.devtooie/debug.log`.
- **Skill state** → `node_modules/.devtooie/skill.json` (installed path + last-written
  version; losing it on reinstall just triggers a harmless skill re-render). §15.

**Generated, git-ignored, at repo root** (must be visible to the consumer's tsconfig):
- **Generated types** → `devtooie-env.d.ts`. §5.4.

So the consumer's only gitignore addition is **`devtooie-env.d.ts`** — everything else
either is committed or lives under the already-ignored `node_modules/`.
...
5. **Project config in `devtooie.yaml`** — services path + API port come from the
   consumer's committed `devtooie.yaml`, not env vars, env files, or CLI flags. No
   `dotenv`; no `DEVTOOIE_API_PORT`.
6. **Writable state** — selection/debug/log/skill-state files live under
   `node_modules/.devtooie/`, never inside the `devtooie` package's own dir (§10).
7. **Port sweep** — only config-derivable ports + the API port are swept (§9.5).
8. **Naming** — project config `devtooie.yaml`, debug flag `DEBUG_DEVTOOIE`, default
   logfile `node_modules/.devtooie/devtooie.log`, scratch dir `node_modules/.devtooie/`.
...
devtooie is meant to be driven by coding agents — both **operating** a running session
(restart/rebuild the service they're editing, check status) and **onboarding** an app
into devtooie (wiring its npm scripts + adding it to the services file). To make that
turnkey, devtooie ships an **agent skill** and installs it during first-time setup. Two
entry points share one flow.
...
- **Frontmatter `description`** must trigger on both driving a session *and* onboarding
  an app — e.g. "…use when running/building/restarting a local dev service, or when
  asked to add/configure/onboard an app or service into devtooie."
- **Content** (generic for v1) teaches an agent three things:
  1. **Invoke headlessly** — always `--plain` with explicit `-s <service>` (no TTY for
     the TUI); `--build`/`--rebuild` to (re)build; how to stop a session.
  2. **Drive a running session via the control API** — read the port from
     ...
     `GET /query/services?status=…`. Discover service names from the running session
     or `devtooie resolvedeps -s <name>` rather than hardcoding them.
  3. **Onboard an app into devtooie** — when the user asks to add/configure/onboard one
     of their apps, the agent should:
     - **Ensure the app has the npm scripts devtooie drives.** For a Node app (has a
       `package.json`), make sure it defines `dev`, `build`, `clean`, and `build:clean`
       (`build:clean` = `clean` then `build`; devtooie's rebuild runs it, and
       `getRebuildable()`/the `b` hotkey key off its presence — §9.4). A non-Node app
       is driven by a `Makefile` with equivalent targets instead.
     - **Rename equivalent existing scripts rather than duplicate.** If the app already
       has a script that does the same job under a different name (e.g. `start:dev` or
       `serve` → `dev`, `compile`/`tsc` → `build`, `rimraf dist` → `clean`), rename it
       to devtooie's name (and update any references), instead of adding a second one.
     - **Add the entry to the services file** — append a new `{ name, types, run }` to
       the `apps` array in the module named by `devtooie.yaml` `services`, inferring
       `types` (`backend`/`browser`/`lib`) and a `run` block (port, healthcheck, deps)
       from the app. Keep `name` consistent with how the app should be referenced.
     - After onboarding, `devtooie typegen` refreshes the type augmentation so the new
       name is known (usually automatic on the next run).
...
- `node_modules/.devtooie/skill.json` records the installed path + the
  version/content-hash devtooie last wrote.
...
- `project-config` read/write round-trip; `getApiPort` (`devtooie.yaml` `apiPort` ›
  default `4099`).
...
6. **Setup + agent skill** — `init.ts` (interactive flow, `@clack/prompts`, scaffolds
   ...
   `assets/skill.md`, `node_modules/.devtooie/skill.json` state), `postinstall.mjs` (CI/TTY-gated
